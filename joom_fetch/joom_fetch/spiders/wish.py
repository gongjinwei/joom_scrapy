...
from joom_fetch.items import WishProductItem,WishProductSkuItem
from fetch.models import WishShopItemUrl
...
class WishSpider(RedisSpider):
    ...
    def make_request_from_data(self, data):
        ...
        cid=urlsplit(item_url).path.split('/')[-1]
        return scrapy.FormRequest('https://www.wish.com/api/product/get', callback=self.parse,
                                  formdata={'cid': cid,'request_sizing_chart_info':'true','do_not_track':'true'},
                                  headers=self.headers, cookies=self.cookies,dont_filter=True,meta={'cid':cid})

    def parse(self, response):
        cid = response.meta.get('cid')

        if response.status > 300:
            WishShopItemUrl.objects.filter(source_id=cid).update(state=3)
        ...
        for variant in variants:
            ...
            yield sku
        yield item_p
        WishShopItemUrl.objects.filter(source_id=cid).update(state=2)